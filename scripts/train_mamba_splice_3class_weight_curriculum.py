...
class MambaSpliceSoftExistPredictor(nn.Module):
    def __init__(
        ...
        if split_output_heads:
            raise ValueError("--split-output-heads is not supported in the 3-class version.")
        self.output_head = nn.Linear(hidden_dim, len(CLASS_NAMES))
...
def evaluate(
    ...
    for val_dna, val_labels, val_sequences in fixed_batches:
        ...
        losses.append(
            splice_loss(
                ...
                cropped_labels,
                cropped_mask,
                positive_weight=args.positive_weight if positive_weight_override is None else positive_weight_override,
                none_weight=args.none_weight,
            )
        )
        logits_rows.append(val_logits)
        binary_logits_rows.append(class_logits_to_binary_logits(val_logits))
    ...
    val_metrics = class_metrics(logits, labels, mask)
    ...
    torch.save(
        {
            ...
            "binary_track_names": TRACK_NAMES,
        },
        path,
    )


def scheduled_positive_weight(args: argparse.Namespace, step: int, total_steps: int) -> float:
    start = args.positive_weight_start if args.positive_weight_start is not None else args.positive_weight
    end = args.positive_weight_end if args.positive_weight_end is not None else start
    if total_steps <= 1 or args.positive_weight_anneal_frac <= 0:
        return float(end)
    anneal_steps = max(1, int(round(total_steps * args.positive_weight_anneal_frac)))
    progress = min(1.0, step / float(anneal_steps))
    if args.positive_weight_schedule == "linear":
        mix = 1.0 - progress
    elif args.positive_weight_schedule == "cosine":
        mix = 0.5 * (1.0 + math.cos(math.pi * progress))
    else:
        raise ValueError(f"Unsupported positive weight schedule: {args.positive_weight_schedule}")
    return float(end + (start - end) * mix)


def lr_for_step(args: argparse.Namespace, step: int, total_steps: int) -> float:
    ...
    denom = max(1, total_steps - max(0, args.warmup_steps))
    progress = min(1.0, max(0, step - args.warmup_steps) / float(denom))
...
def validation_score(args: argparse.Namespace, val_loss: float, metrics_out: dict[str, float]) -> float:
    if args.best_metric == "val_loss":
        return -val_loss
    if args.best_metric == "ap_mean":
        return 0.5 * (metrics_out["don_ap"] + metrics_out["acc_ap"])
    if args.best_metric == "topk_mean":
        return 0.5 * (metrics_out["don_topk"] + metrics_out["acc_topk"])
    if args.best_metric == "top2k_mean":
        return 0.5 * (metrics_out["don_top2k_rec"] + metrics_out["acc_top2k_rec"])
...
def train(args: argparse.Namespace) -> None:
    ...
    scheduler = (
        ...
        if milestones and args.lr_schedule == "multistep"
        else None
    )
    ...
    print("Mamba 3-class splice predictor with positive-weight/LR curriculum")
    print(
    ...
    print(
        ...
        f"junk/base={args.junk_slots_per_base}; classes={CLASS_NAMES}; "
        f"loss weights none/positive={args.none_weight}/{args.positive_weight}; "
        f"positive_weight_start/end={args.positive_weight_start}/{args.positive_weight_end}; "
        f"positive_weight_schedule={args.positive_weight_schedule}; "
    ...
    print(
        ...
        f"lr_schedule={args.lr_schedule}; warmup_steps={args.warmup_steps}; min_lr_ratio={args.min_lr_ratio}; "
        f"epochs={args.epochs}; steps_per_epoch={args.steps_per_epoch}; lr_milestones={milestones}; "
        f"best_metric={args.best_metric}"
    )

    for step in range(total_steps):
        current_lr = lr_for_step(args, step, total_steps)
        if args.lr_schedule == "cosine":
            set_optimizer_lr(optimizer, current_lr)
        current_positive_weight = scheduled_positive_weight(args, step, total_steps)
        ...
        loss = splice_loss(
            ...
            labels_train,
            mask_train,
            positive_weight=current_positive_weight,
            none_weight=args.none_weight,
        )
        ...
        if step % args.print_every == 0 or step == total_steps - 1 or end_of_epoch:
            model.eval()
            with torch.no_grad():
                eval_positive_weight = current_positive_weight if args.val_loss_uses_current_weight else args.positive_weight
                val_loss, val_metrics, val_motifs, val_input_len, val_exist_sum, val_aug = evaluate(
                    args,
                    model,
                    fixed_val_batches,
                    positive_weight_override=eval_positive_weight,
                )
            score = validation_score(args, val_loss, val_metrics)
            if score > best_score:
                best_score = score
                best_loss = val_loss
            ...
            print(
                ...
                f"lr {optimizer.param_groups[0]['lr']:.2e} pos_w {current_positive_weight:.1f} loss {loss.item():.4f} "
                f"val {val_loss:.4f} best {best_loss:.4f} best_{args.best_metric} {best_score:.4f} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a Mamba 3-class splice-site predictor with positive-weight and LR curricula.")
    ...
    parser.add_argument("--positive-weight-start", type=float, default=None)
    parser.add_argument("--positive-weight-end", type=float, default=None)
    parser.add_argument("--positive-weight-anneal-frac", type=float, default=1.0)
    parser.add_argument("--positive-weight-schedule", choices=("linear", "cosine"), default="cosine")
    parser.add_argument("--none-weight", type=float, default=1.0)
    parser.add_argument("--best-metric", choices=("val_loss", "ap_mean", "topk_mean", "top2k_mean"), default="ap_mean")
    parser.add_argument("--val-loss-uses-current-weight", action="store_true")
    parser.add_argument("--lr-schedule", choices=("multistep", "cosine"), default="multistep")
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/mamba_splice_3class_weight_curriculum")
