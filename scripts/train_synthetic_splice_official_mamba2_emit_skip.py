...
class MambaEmitSkipTranslator(nn.Module):
    ...
    def forward(self, dna_one_hot: torch.Tensor, splice_tracks: torch.Tensor, transcript_bases: int):
        ...
        emit_gate = exon_prior.squeeze(-1).clamp(0, 1) if self.use_prior_emit_mask else torch.ones_like(emit_logits)
        ...
        mean_exon_assignment = torch.einsum(
            "btl,bl->bt", assignment, exon_prior.squeeze(-1)
        ).mean()
        ...
        return amino_acid_probs, transcript_base_probs, assignment, assignment_logits, {
            ...
            "mean_exon_assignment": mean_exon_assignment.detach(),
        }
...
def evaluate_model_batch(
    *,
    ...
    return {
        ...
        "mean_exon_assignment": float(diagnostics["mean_exon_assignment"].item()),
        **failure,
    }
...
def train(args: argparse.Namespace) -> None:
    ...
    for step in range(start_step, args.steps):
        ...
        for micro_batch_index in range(micro_batch_count):
            ...
            with torch.no_grad():
                ...
                mean_exon_assignment_sum += diagnostics["mean_exon_assignment"].item() * micro_batch_size
        ...
        mean_exon_assignment = mean_exon_assignment_sum / max(1, total_examples)
        ...
        final_metrics = {
            ...
            "mean_exon_assignment": mean_exon_assignment,
        ...
        if is_report_step:
            print(
                ...
                f"exon_assign={mean_exon_assignment:.3f}",
                flush=True,
            )
            print(
            ...
            if validation_metrics is not None:
                print(
                    ...
                    f"exon_assign={validation_metrics['mean_exon_assignment']:.3f}",
                    flush=True,
                )
                print(
    ...
    print("mean exon assignment:", diagnostics["mean_exon_assignment"].item())


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a monotonic emit/skip synthetic splice renderer with official mamba-ssm Mamba2 on CUDA."
    )
    ...
    parser.add_argument("--use-prior-emit-mask", action=argparse.BooleanOptionalAction, default=True)
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/synthetic_splice_official_mamba2_emit_skip")
