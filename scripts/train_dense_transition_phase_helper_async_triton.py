...
if triton is not None:
    @triton.jit
    def hmm_forward_scan_kernel(
        T_probs_ptr, states_out_ptr,
        B, L,
        stride_tb, stride_tl, stride_tr, stride_tc,
        stride_sb, stride_sl, stride_ss,
        initial_state: tl.constexpr,
        S_PAD: tl.constexpr,
    ):
        pid = tl.program_id(0)

        # Calculate base pointers for this specific batch element
        t_ptr = T_probs_ptr + pid * stride_tb
        s_ptr = states_out_ptr + pid * stride_sb

        # Setup row and column indices for a block load
        idx = tl.arange(0, S_PAD)
        col_idx = tl.arange(0, S_PAD)

        # Initialise the state vector in SRAM (one-hot)
        state = tl.where(idx == initial_state, 1.0, 0.0)

        # Pre-compute the memory offsets for the 32x32 transition matrix block
        offsets = idx[:, None] * stride_tr + col_idx[None, :] * stride_tc

        for t in range(L):
            # Load the dense transition matrix for this nucleotide step
            T_mat = tl.load(t_ptr + t * stride_tl + offsets)

            # Compute next state: state @ T_mat
            # Expanding state to [32, 1] broadcasts it down the columns for element-wise multiplication
            state_expanded = state[:, None]
            state = tl.sum(state_expanded * T_mat, axis=0)

            # Store the resulting state back to global memory
            tl.store(s_ptr + t * stride_sl + idx * stride_ss, state)
else:
    hmm_forward_scan_kernel = None
...
class DenseTransitionPhaseLayer(nn.Module):
    ...
    def forward(self, evidence_logits: torch.Tensor) -> DenseLayerOutput:
        ...
        if triton is None or evidence_logits.device.type != 'cuda':
            ...
            # 1. Fully vectorise the edge probability computation 
            edge_log_probs = self.edge_log_probs(evidence_logits)
            edge_probs = edge_log_probs.exp()

            # 2. Construct dense padded transition matrices for the whole sequence
            # S is 24, pad to 32 to satisfy Triton's power-of-two constraints
            S_PAD = 32
            T_dense = torch.zeros((batch, length, S_PAD, S_PAD), device=evidence_logits.device, dtype=torch.float32)
            T_dense[:, :, self.edge_from, self.edge_to] = edge_probs

            # 3. Execute the fused Triton scan kernel
            states_padded = torch.empty((batch, length, S_PAD), device=evidence_logits.device, dtype=torch.float32)

            grid = lambda meta: (batch,)
            hmm_forward_scan_kernel[grid](
                T_dense, states_padded,
                batch, length,
                T_dense.stride(0), T_dense.stride(1), T_dense.stride(2), T_dense.stride(3),
                states_padded.stride(0), states_padded.stride(1), states_padded.stride(2),
                initial_state=self.initial_state,
                S_PAD=S_PAD
            )

            # Slice away the padding to return to S=24
            state_probs = states_padded[:, :, :S]

            # 4. Vectorise the calculation of transition type posteriors
            # To calculate the marginals at step t, we need the state from step t-1
            initial_states = torch.zeros((batch, 1, S), device=evidence_logits.device)
            initial_states[:, 0, self.initial_state] = 1.0
            states_before = torch.cat([initial_states, state_probs[:, :-1, :]], dim=1)

            # Compute marginals globally instead of iteratively
            edge_marginals = states_before[:, :, self.edge_from] * edge_probs
            
            transition_type_posteriors = torch.zeros((batch, length, K), device=evidence_logits.device)
            edge_type_expanded = self.edge_type.view(1, 1, -1).expand(batch, length, -1)
            transition_type_posteriors.scatter_add_(2, edge_type_expanded, edge_marginals)
