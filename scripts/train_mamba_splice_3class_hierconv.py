...
class HierarchicalConvResidual(nn.Module):
    def __init__(self, hidden_dim: int, kernel_size: int, dropout: float):
        super().__init__()
        if kernel_size < 1 or kernel_size % 2 == 0:
            raise ValueError("Layer conv kernels must be positive odd integers.")
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        return (x + self.dropout(y)) * mask[..., None]


def expand_kernel_schedule(raw: str, layers: int) -> tuple[int, ...]:
    kernels = parse_int_list(raw)
    for kernel in kernels:
        if kernel < 1 or kernel % 2 == 0:
            raise ValueError("--layer-conv-kernels must contain positive odd integers.")
    if len(kernels) >= layers:
        return kernels[:layers]
    return kernels + (kernels[-1],) * (layers - len(kernels))
...
class MambaSpliceSoftExistPredictor(nn.Module):
    def __init__(
        ...
        layer_conv_kernels: tuple[int, ...],
    ):
        super().__init__()
        self.split_output_heads = split_output_heads
        if local_conv_kernel > 0 and local_conv_kernel % 2 == 0:
            raise ValueError("--local-conv-kernel must be odd, or 0 to disable the final high-level local head.")
        ...
        if len(layer_conv_kernels) != layers:
            raise ValueError("--layer-conv-kernels must expand to exactly --layers kernels.")
        self.layer_convs = nn.ModuleList(
            [HierarchicalConvResidual(hidden_dim, kernel_size=kernel, dropout=dropout) for kernel in layer_conv_kernels]
        )
        self.layer_conv_kernels = tuple(layer_conv_kernels)
    ...
    def forward(self, dna_probs: torch.Tensor, existence: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        x = x * mask[..., None]
        for block, conv in zip(self.blocks, self.layer_convs, strict=True):
            x = block(x, mask)
            x = conv(x, mask)
...
def train(args: argparse.Namespace) -> None:
    ...
    model = MambaSpliceSoftExistPredictor(
        ...
        layer_conv_kernels=expand_kernel_schedule(args.layer_conv_kernels, args.layers),
    ).to(device)
    ...
    print("Hierarchical-conv Mamba 3-class splice predictor with soft DNA + existence inputs")
    print(
        ...
        f"layer_conv_kernels={','.join(str(k) for k in model.layer_conv_kernels)}; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a hierarchical-conv Mamba 3-class splice-site predictor on GTF/FASTA.")
    ...
    parser.add_argument("--layer-conv-kernels", default="1,3,3,5,7,9", help="Odd per-layer conv kernels. If fewer than --layers, the last value is repeated.")
    parser.add_argument("--local-conv-kernel", type=int, default=0, help="Optional final high-level local conv head. Use 0 to disable.")
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/mamba_splice_3class_hierconv")
