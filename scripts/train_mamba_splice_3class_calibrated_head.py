...
class CalibratedSpliceHead(nn.Module):
    """High-level local readout with explicit site-vs-none calibration.

    The body still emits one hidden state per position. This head sharpens the
    local decision using centered convolutions over hidden states, then factors
    the final probabilities into:

      P(none)
      P(site) * P(acceptor | site)
      P(site) * P(donor | site)
    """

    def __init__(self, hidden_dim: int, kernel_size: int, depth: int, dropout: float):
        super().__init__()
        if kernel_size < 1 or kernel_size % 2 == 0:
            raise ValueError("--head-kernel must be a positive odd integer.")
        if depth < 1:
        ...
        for _ in range(depth):
            layers.extend(
                [
                    ...
                    nn.Conv1d(hidden_dim, hidden_dim, kernel_size=kernel_size, padding=padding, groups=hidden_dim),
                    nn.GELU(),
        ...
        self.type_head = nn.Linear(hidden_dim, 2)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        while index < len(self.layers):
            residual = y
            y_norm = self.layers[index](y)
            index += 1
            y_conv = self.layers[index](y_norm.transpose(1, 2))
            index += 1
            y_conv = self.layers[index](y_conv)
            index += 1
            y_conv = self.layers[index](y_conv).transpose(1, 2)
            index += 1
            y_conv = self.layers[index](y_conv)
            index += 1
            y = (residual + self.dropout(y_conv)) * mask[..., None]
        scale = self.logit_scale.clamp(0.25, 8.0)
        site_logit = self.site_head(y).squeeze(-1) * scale
        type_logits = self.type_head(y) * scale
        log_none = F.logsigmoid(-site_logit)
        log_site = F.logsigmoid(site_logit)
...
class MambaSpliceSoftExistPredictor(nn.Module):
    def __init__(
        ...
        self.output_head = CalibratedSpliceHead(
...
def splice_loss(
    ...
    if focal_gamma > 0:
        probs = class_logits.softmax(dim=-1)
        pt = probs.gather(dim=-1, index=targets[..., None]).squeeze(-1).clamp(1e-6, 1.0)
        raw = raw * (1.0 - pt).pow(focal_gamma)
...
def train(args: argparse.Namespace) -> None:
    ...
    print("Mamba 3-class splice predictor with calibrated local readout")
    print(
        ...
        f"head_kernel={args.head_kernel}; head_depth={args.head_depth}; "
    ...
    print(
        ...
        f"loss weights none/positive={args.none_weight}/{args.positive_weight}; focal_gamma={args.focal_gamma}; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a Mamba 3-class splice-site predictor on GTF/FASTA with soft DNA and existence-column augmentation.")
    ...
    parser.add_argument("--head-kernel", type=int, default=9, help="Odd kernel size for the calibrated high-level local readout.")
    parser.add_argument("--head-depth", type=int, default=2, help="Number of calibrated local readout residual blocks.")
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/mamba_splice_3class_calibrated_head")
