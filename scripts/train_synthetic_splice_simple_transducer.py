...
def make_batch(
    ...
    max_exon_count: int = 3,
    min_exon_bases: int = 6,
    min_intron_length: int = 4,
...
class SimpleSpliceRenderer(nn.Module):
    def __init__(self, use_exon_calibrator: bool = True, hidden_dim: int = 16):
        super().__init__()
        self.use_exon_calibrator = use_exon_calibrator
        if use_exon_calibrator:
            self.exon_calibrator = nn.Sequential(
                nn.Linear(6, hidden_dim),
                nn.GELU(),
                nn.Linear(hidden_dim, 1),
            )
        else:
            self.exon_calibrator = None
        self.coordinate_log_sharpness = nn.Parameter(torch.tensor(2.0))
        self.exon_logit_scale = nn.Parameter(torch.tensor(4.0))
    ...
    def forward(
        ...
        exon_prior = splice_tracks[..., 0].clamp(0.0, 1.0)

        genome_position = torch.linspace(0, 1, genome_length, device=dna_one_hot.device, dtype=dna_one_hot.dtype)
        genome_position = genome_position[None, :, None].expand(batch_size, -1, -1)
        if self.exon_calibrator is not None:
            exon_features = torch.cat([dna_one_hot, splice_tracks, genome_position], dim=-1)
            calibrated_logit = self.exon_calibrator(exon_features).squeeze(-1)
            exon_probability = torch.sigmoid(calibrated_logit + self.exon_logit_scale * (exon_prior * 2.0 - 1.0))
        else:
            exon_probability = exon_prior

        coordinate_step = exon_probability.clamp_min(1e-6)
        packed_coordinate = coordinate_step.cumsum(dim=1)
        packed_coordinate = packed_coordinate / packed_coordinate[:, -1:].clamp_min(1e-6)

        target_index = torch.arange(transcript_bases, device=dna_one_hot.device)
        target_coordinate = (target_index.to(dna_one_hot.dtype) + 1.0) / float(transcript_bases)
        coordinate_sharpness = F.softplus(self.coordinate_log_sharpness)
        coordinate_bias = -coordinate_sharpness * (
            packed_coordinate[:, None, :] - target_coordinate[None, :, None]
        ).pow(2)

        exon_bias = torch.log(exon_probability.clamp_min(1e-5))[:, None, :]
        pointer_logits = coordinate_bias + exon_bias
        attention = pointer_logits.softmax(dim=-1)
        transcript_base_probs = torch.einsum("btl,blc->btc", attention, dna_one_hot).clamp_min(1e-8)
        transcript_base_probs = transcript_base_probs / transcript_base_probs.sum(dim=-1, keepdim=True).clamp_min(1e-8)
        ...
        attention_entropy = -(attention.clamp_min(1e-8) * attention.clamp_min(1e-8).log()).sum(dim=-1).mean()
        mean_exon_attention = torch.einsum("btl,bl->bt", attention, exon_prior).mean()
        coordinate_span = (packed_coordinate.max(dim=1).values - packed_coordinate.min(dim=1).values).mean()
        mean_exon_probability = exon_probability.mean()
        ...
        return amino_acid_probs, transcript_base_probs, attention, pointer_logits, {
            ...
            "mean_exon_probability": mean_exon_probability.detach(),
        }
...
def get_linear_annealed_weight(step: int, start_weight: float, end_step: int) -> float:
    if start_weight <= 0 or end_step <= 0 or step >= end_step:
        return 0.0
...
def train(args: argparse.Namespace) -> None:
    ...
    print("renderer: no Mamba, no sequence encoder; exon probability -> cumsum coordinate -> pointer")
    print(
    ...
    model = SimpleSpliceRenderer(
        use_exon_calibrator=not args.no_exon_calibrator,
    ...
    if args.checkpoint_every > 0 or args.save_best_checkpoint:
    ...
    for step in range(start_step, args.steps):
        ...
        final_metrics = {
            ...
            "mean_exon_probability": diagnostics["mean_exon_probability"].item(),
        }
        ...
        if is_report_step:
            print(
                f"step={step:04d} lr={lr:.2e} loss={loss.item():.3f} "
                ...
                f"exon_prob={diagnostics['mean_exon_probability'].item():.3f} "
                f"exon_attention={diagnostics['mean_exon_attention'].item():.3f}",
                flush=True,
            )

        if args.checkpoint_every > 0 and ((step + 1) % args.checkpoint_every == 0 or is_final_step):
            latest_path = checkpoint_dir / "latest.pt"
            save_checkpoint(
                latest_path,
                checkpoint_payload(
    ...
    print("mean exon probability:", diagnostics["mean_exon_probability"].item())


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a no-Mamba differentiable splice renderer/transducer on synthetic examples."
    )
    parser.add_argument("--device", default="auto", help="Device to use. auto prefers MPS, then CUDA, then CPU.")
    parser.add_argument("--steps", type=int, default=10_000)
    parser.add_argument("--batch-size", type=int, default=8)
    parser.add_argument("--min-protein-codons", type=int, default=8)
    parser.add_argument("--max-protein-codons", type=int, default=32)
    parser.add_argument("--min-exon-count", type=int, default=1)
    parser.add_argument("--max-exon-count", type=int, default=3)
    parser.add_argument("--min-exon-bases", type=int, default=6)
    parser.add_argument("--eval-protein-codons", type=int, default=16)
    parser.add_argument("--eval-exon-count", type=int, default=2)
    parser.add_argument("--min-intron-length", type=int, default=4)
    parser.add_argument("--max-intron-length", type=int, default=36)
    parser.add_argument("--length-bucket-size", type=int, default=64)
    parser.add_argument("--hidden-dim", type=int, default=16)
    parser.add_argument("--no-exon-calibrator", action="store_true")
    ...
    parser.add_argument("--pointer-loss-anneal-steps", type=int, default=2_000)
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/synthetic_splice_simple_transducer")
