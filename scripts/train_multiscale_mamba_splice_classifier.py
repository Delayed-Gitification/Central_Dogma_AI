...
class MambaSpliceSoftExistPredictor(nn.Module):
    def __init__(
        ...
        multiscale_layers: int,
        multiscale_stride: int,
    ):
        super().__init__()
        if local_conv_kernel > 0 and local_conv_kernel % 2 == 0:
            raise ValueError("--local-conv-kernel must be odd, or 0 to disable the high-level local head.")
        if multiscale_stride < 1:
            raise ValueError("--multiscale-stride must be >= 1.")
        ...
        self.multiscale_stride = multiscale_stride
        self.coarse_blocks = nn.ModuleList(
            [block_cls(hidden_dim, chunk_size=chunk_size, headdim=headdim) for _ in range(multiscale_layers)]
        )
    ...
    def _coarse_stream(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        if not self.coarse_blocks or self.multiscale_stride <= 1 or x.shape[1] < self.multiscale_stride:
            return x
        stride = self.multiscale_stride
        weighted = x * mask[..., None]
        coarse_mask = F.max_pool1d(mask[:, None, :], kernel_size=stride, stride=stride, ceil_mode=True).squeeze(1)
        coarse_sum = F.avg_pool1d(
        ...
        coarse_denom = F.avg_pool1d(
            mask[:, None, :],
            kernel_size=stride,
            stride=stride,
            ceil_mode=True,
            count_include_pad=False,
        ).transpose(1, 2).clamp_min(1e-6)
        coarse = coarse_sum / coarse_denom
        for block in self.coarse_blocks:
            coarse = block(coarse, coarse_mask)
        ...
        return x + self.coarse_fuse(torch.cat([x, coarse_up], dim=-1)) * mask[..., None]
...
def train(args: argparse.Namespace) -> None:
    ...
    model = MambaSpliceSoftExistPredictor(
        ...
        multiscale_stride=args.multiscale_stride,
    ).to(device)
    ...
    print("Multiscale Mamba splice classifier with soft DNA + existence inputs")
    print(
        f"device={device}; seq_len={args.seq_len}; target_len={args.target_length}; "
        f"batch={args.batch_size}; hidden={args.hidden_dim}; "
        f"layers={args.layers}; coarse_layers={args.multiscale_layers}; coarse_stride={args.multiscale_stride}; "
        f"bidirectional={not args.unidirectional}; high_level_local_kernel={args.local_conv_kernel}; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a multiscale Mamba 3-class splice-site classifier on GTF/FASTA windows.")
    ...
    parser.add_argument("--multiscale-layers", type=int, default=2)
    parser.add_argument("--multiscale-stride", type=int, default=8)
    ...
    parser.add_argument("--local-conv-kernel", type=int, default=9, help="Odd kernel size for a high-level local refinement head. Use 0 to disable.")
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints/multiscale_mamba_splice_classifier")
