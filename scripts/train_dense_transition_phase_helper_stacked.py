...
class StackedDenseTransitionPhaseModel(nn.Module):
    def __init__(self, *, hidden_dim: int, conv_layers: int, use_splice_tracks: bool, materialize_transitions: bool):
        ...
        # Unit 2 (takes input_dim1 + 24 state_probs + 8 transition_type_posteriors)
...
def evaluate(model: StackedDenseTransitionPhaseModel, args: argparse.Namespace, *, device: torch.device, seed: int, examples: int) -> dict[str, object]:
    ...
    while remaining > 0:
        ...
        for key in loss_sums:
            loss_sums[key] += 0.5 * (float(parts1[key].item()) + float(parts2[key].item())) * chunk_size
        output = output2
...
def main() -> None:
    ...
    for step in range(start_step, args.steps + 1):
        ...
        output = output2
        parts = {key: 0.5 * (parts1[key] + parts2[key]) for key in parts1}
        if should_report:
            with torch.no_grad():
                predicted = output.state_log_probs.argmax(dim=-1)
                matches = (predicted == batch.target_states) & batch.mask
