...
def configure_mamba_cache() -> Path:
    ...
    cache_root = Path(
        ...
        or ROOT / ".cache" / "train_dense_transition_phase_helper_attention_extra_positional_utr_introns_local_conv_mixers"
    ).expanduser()
    cache_dirs = {
...
def build_edge_type_matrix() -> torch.Tensor:
    ...
    for g in range(3):
        ...
        add(idx_U(g), idx_I(g_next, 0), "donor")
        add(idx_T(g), idx_T(g_next), "u3")
        add(idx_T(g), idx_I(g_next, 0), "donor")
        for p in range(3):
            p_next = (p + 1) % 3
        ...
        add(idx_I(g, 0), idx_U(g_next), "acceptor")
        add(idx_I(g, 0), idx_T(g_next), "acceptor")
...
def sample_utr_cut_points(length: int, max_introns: int, probability: float, rng: random.Random) -> tuple[int, ...]:
    if max_introns <= 0 or length < 2 or rng.random() > probability:
        return ()
    intron_count = rng.randint(1, min(max_introns, length - 1))
    return tuple(sorted(rng.sample(range(1, length), intron_count)))


def exon_lengths_from_rows(rows: list[tuple[str, str, int | None]]) -> tuple[int, ...]:
    lengths = []
    current = 0
    for _base, region, _phase in rows:
        if region == "I":
            if current:
...
def block_same_frame_utr5_atgs(rows: list[tuple[str, str, int | None]], start_pos: int, rng: random.Random) -> None:
    start_frame = start_pos % 3
    for pos in range(0, max(0, start_pos - 2)):
        if pos % 3 != start_frame:
            continue
        triplet = rows[pos : pos + 3]
        if any(region != "U" for _base, region, _phase in triplet):
            continue
        if "".join(base for base, _region, _phase in triplet) == "ATG":
            base, region, phase = rows[pos + 2]
            rows[pos + 2] = (rng.choice(("A", "C", "T")), region, phase)


def next_state_for_transition(current_state: int, type_index: int, next_region: str) -> int:
    candidates = torch.nonzero(EDGE_TYPE_MATRIX[current_state] == type_index, as_tuple=False).flatten().tolist()
    if not candidates:
        return -1
    if len(candidates) == 1:
        return int(candidates[0])
    if type_index == TYPE_TO_INDEX["acceptor"]:
        if next_region == "U":
            candidates = [state for state in candidates if 0 <= state <= 2]
        elif next_region == "C":
            candidates = [state for state in candidates if 3 <= state <= 11]
        elif next_region == "T":
            candidates = [state for state in candidates if 21 <= state <= 23]
    if len(candidates) != 1:
        raise RuntimeError(
            f"Ambiguous transition from {STATE_NAMES[current_state]} using "
            f"{TRANSITION_TYPES[type_index]} into region {next_region}: {candidates}"
        )
    return int(candidates[0])
...
def generate_dense_gene(
    *,
    ...
    intron_mod: str = "any",
    allow_utr_introns: bool = False,
    max_utr5_introns: int = 0,
    max_utr3_introns: int = 0,
    utr_intron_probability: float = 0.0,
    seed: int = 1,
) -> DenseGene:
    if coding_codons < 2:
    ...
    cds_cuts = sample_cds_cut_points(len(cds), exon_count - 1, min_exon_length, rng)
    utr5_cuts = sample_utr_cut_points(utr5_length, max_utr5_introns, utr_intron_probability, rng) if allow_utr_introns else ()
    utr3_cuts_local = sample_utr_cut_points(utr3_length, max_utr3_introns, utr_intron_probability, rng) if allow_utr_introns else ()

    mature_rows: list[tuple[str, str, int | None]] = []
    mature_rows.extend((base, "U", None) for base in utr5)
    mature_start_index = len(mature_rows)
    for cds_offset, base in enumerate(cds):
        mature_rows.append((base, "C", cds_offset % 3))
    mature_stop_index = mature_start_index + len(cds) - 3
    mature_utr3_start = len(mature_rows)
    mature_rows.extend((base, "T", None) for base in utr3)

    cut_points: dict[int, int] = {}
    for cut in utr5_cuts:
        cut_points[cut] = 0
    for cut in cds_cuts:
        cut_points[mature_start_index + cut] = cut % 3
    for cut in utr3_cuts_local:
        cut_points[mature_utr3_start + cut] = 0
    ...
    for mature_index, row in enumerate(mature_rows):
        if mature_index == mature_start_index:
            start_pos = len(rows)
        if mature_index == mature_stop_index:
            stop_pos = len(rows)
        rows.append(row)
        cut = mature_index + 1
        if cut in cut_points:
            ...
            carried_phase = cut_points[cut]
            rows.extend((base, "I", carried_phase) for base in intron)
            acceptor_positions.append(len(rows))
    if start_pos < 0 or stop_pos < 0:
        raise RuntimeError("Could not locate start/stop positions after intron insertion")
    block_same_frame_utr5_atgs(rows, start_pos, rng)
    exon_lengths = exon_lengths_from_rows(rows)
    ...
    for pos, transition_type in enumerate(transition_types):
        ...
        next_state = next_state_for_transition(current_state, type_index, rows[pos][1])
        if next_state == -1:
            raise RuntimeError(
...
class BiMambaBlock(nn.Module):
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4, expand: int = 2):
        ...
        try:
            ...
            # Fallback to GRU for local Mac/CPU CPU checks and prototyping
            class BiMambaFallback(nn.Module):
                def __init__(self):
                    super().__init__()
                    self.gru = nn.GRU(d_model, d_model // 2, num_layers=1, bidirectional=True, batch_first=True)
                def forward(self, x):
                    out, _ = self.gru(x)
                    return out
            Mamba = None
        ...
        if Mamba is not None:
            ...
            self.mamba_fwd = BiMambaFallback()
            self.mamba_bwd = None
...
class AttentionRefinementBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        use_local_refinement_stem: bool = True,
        local_refinement_init_scale: float = 0.0,
        ...
        self.use_local_refinement_stem = use_local_refinement_stem
        self.mixer_type = mixer_type
        self.last_extra_input_norm: torch.Tensor | None = None
        self.last_extra_output_norm: torch.Tensor | None = None
        self.last_x_base_norm: torch.Tensor | None = None
        self.last_x_local_norm: torch.Tensor | None = None
        ...
        if mixer_type in ("mamba", "mamba_attention"):
            self.mamba_blocks = nn.ModuleList([
                BiMambaBlock(d_model=attention_dim, d_state=mamba_d_state, d_conv=mamba_d_conv, expand=mamba_expand)
    ...
    def forward(self, features1: torch.Tensor, evidence1_logits: torch.Tensor, output1_fw: DenseLayerOutput) -> torch.Tensor:
        ...
        x_base = self.proj_in(x_conv_in)
        self.last_x_base_norm = x_base.detach().norm()
        ...
        # 1. Apply BiMamba if selected (at native resolution)
        if self.mixer_type in ("mamba", "mamba_attention") and self.mamba_blocks is not None:
            # Mamba layer expects (B, L, attention_dim)
            x_mamba = x_base.transpose(1, 2)
            for block in self.mamba_blocks:
                x_mamba = block(x_mamba)
            x_base = x_mamba.transpose(1, 2)
            
        # 2. Apply downsampled attention if selected
        if self.mixer_type in ("attention", "mamba_attention"):
            ...
            # If "mamba" only (already at native resolution)
            x_fused = x_base
...
class StackedDenseTransitionPhaseModel(nn.Module):
    def __init__(self, *, 
        ...
        if use_attention_refinement:
            self.refiner = AttentionRefinementBlock(
                ...
                use_local_refinement_stem=use_local_refinement_stem,
                local_refinement_init_scale=local_refinement_init_scale,
            ...
            self.refiner2 = AttentionRefinementBlock(
                input_dim1=input_dim1,
                attention_dim=attention_dim,
                downsample=attention_downsample,
                layers=attention_layers,
            ...
            nn.init.normal_(self.refiner2[-1].weight, std=0.01)
            nn.init.zeros_(self.refiner2[-1].bias)
        else:
            ...
            self.evidence3 = DilatedConvEvidenceModel(
                hidden_dim=hidden_dim,
                layers=conv_layers,
                input_dim=input_dim2,
            )
        ...
        self.phase_layer3 = DenseTransitionPhaseLayer(materialize_transitions=materialize_transitions)
    ...
    def forward(self, dna_one_hot: torch.Tensor, splice_tracks: torch.Tensor | None = None) -> tuple[
        ...
        output2 = self.phase_layer2(evidence2_logits, run_backward=self.use_backward_features)
        
        # Unit 3 Forward (Pass 3)
        delta_logits2 = None
        if self.use_attention_refinement:
            delta_logits2 = self.refiner2(features1, evidence2_logits, output2)
            evidence3_logits = evidence2_logits + self.delta_logit_scale * delta_logits2
        elif self.use_residual_refinement:
            state_probs = output2.state_probs
            state_entropy = - (state_probs * torch.log(state_probs.clamp_min(1.0e-30))).sum(dim=-1, keepdim=True)
            transition_posteriors = output2.transition_type_posteriors
            transition_entropy = - (transition_posteriors * torch.log(transition_posteriors.clamp_min(1.0e-30))).sum(dim=-1, keepdim=True)
            refinement_features = [
                features1,
                evidence2_logits,
            ...
            if self.use_backward_features and output2.beta is not None:
                beta = output2.beta
                beta_entropy = - (beta * torch.log(beta.clamp_min(1.0e-30))).sum(dim=-1, keepdim=True)
                refinement_features.extend([beta, beta_entropy])
            x = torch.cat(refinement_features, dim=-1)
            delta_logits2 = self.refiner2(x.transpose(1, 2)).transpose(1, 2)
            evidence3_logits = evidence2_logits + self.delta_logit_scale * delta_logits2
        else:
            features3 = torch.cat([features1, output2.state_probs, output2.transition_type_posteriors], dim=-1)
            evidence3_logits = self.evidence3(features3)
            
        output3 = self.phase_layer3(evidence3_logits, run_backward=False)
        ...
        if squeeze_batch:
            ...
            return (
                (squeeze_output(output1), evidence1_logits.squeeze(0)), 
                (squeeze_output(output2), evidence2_logits.squeeze(0)), 
                (squeeze_output(output3), evidence3_logits.squeeze(0)), 
                delta_logits.squeeze(0) if delta_logits is not None else None,
                delta_logits2.squeeze(0) if delta_logits2 is not None else None
            )
            
        return (
            (output1, evidence1_logits), 
            (output2, evidence2_logits), 
            (output3, evidence3_logits), 
            delta_logits, 
            delta_logits2
        )
...
def make_gene(args: argparse.Namespace, rng: random.Random) -> DenseGene:
    return generate_dense_gene(
        ...
        allow_utr_introns=args.allow_utr_introns,
        max_utr5_introns=args.max_utr5_introns,
        max_utr3_introns=args.max_utr3_introns,
        utr_intron_probability=args.utr_intron_probability,
...
def evaluate(model: StackedDenseTransitionPhaseModel, args: argparse.Namespace, *, device: torch.device, seed: int, examples: int) -> dict[str, object]:
    ...
    while remaining > 0:
        ...
        (output1, evidence1_logits), (output2, evidence2_logits), (output3, evidence3_logits), delta_logits, delta_logits2 = model(batch.dna_one_hot, batch.splice_tracks if args.use_splice_tracks else None)
        ...
        loss3, parts3 = compute_batch_loss(
            output3,
            evidence3_logits,
            batch,
            evidence_loss_weight=args.evidence_loss_weight,
        ...
        for key in loss_sums:
            loss_sums[key] += (args.layer1_loss_weight * float(parts1[key].item()) + args.layer2_loss_weight * float(parts2[key].item()) + args.layer3_loss_weight * float(parts3[key].item())) * chunk_size
        output = output3
...
def format_extra_attention_stats(model: nn.Module) -> str | None:
    ...
    if hasattr(refiner, "extra_attention_stats"):
        ...
        if not torch.isnan(stats["attn_gate_mean"]):
            lines.append(
                "extra attn | "
        ...
        elif not torch.isnan(stats["pos_encoding_scale"]):
            lines.append(f"extra attn | none; pos_scale={stats['pos_encoding_scale'].item():.4g}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--allow-utr-introns", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--max-utr5-introns", type=int, default=0)
    parser.add_argument("--max-utr3-introns", type=int, default=0)
    parser.add_argument("--utr-intron-probability", type=float, default=0.0)
    ...
    parser.add_argument("--layer3-loss-weight", type=float, default=1.0)
    parser.add_argument("--use-local-refinement-stem", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--local-refinement-init-scale", type=float, default=0.0)
    parser.add_argument("--mixer-type", choices=("attention", "mamba", "mamba_attention"), default="attention")
...
def main() -> None:
    ...
    model = StackedDenseTransitionPhaseModel(
        ...
        use_local_refinement_stem=args.use_local_refinement_stem,
        local_refinement_init_scale=args.local_refinement_init_scale,
    ...
    print(
        f"mixer type: {args.mixer_type} (mamba_layers={args.mamba_layers}, d_state={args.mamba_d_state}, d_conv={args.mamba_d_conv}, expand={args.mamba_expand:g})\n"
    ...
    print(
        "UTR introns: "
        f"enabled={args.allow_utr_introns}, max_utr5={args.max_utr5_introns}, "
        f"max_utr3={args.max_utr3_introns}, probability={args.utr_intron_probability:.3f}"
    )
    ...
    if args.resume_checkpoint and args.resume_checkpoint.exists():
        ...
        # If resuming from a 2-pass checkpoint, clone refiner weights to refiner2
        refiner_keys = [k for k in state_dict if k.startswith("refiner.")]
        for k in refiner_keys:
            refiner2_k = k.replace("refiner.", "refiner2.")
            if refiner2_k not in state_dict:
                state_dict[refiner2_k] = state_dict[k].clone()
    ...
    for step in range(start_step, args.steps + 1):
        ...
        step_loss = args.layer1_loss_weight * loss1 + args.layer2_loss_weight * loss2 + args.layer3_loss_weight * loss3
        output = output3
        parts = {key: args.layer1_loss_weight * parts1[key] + args.layer2_loss_weight * parts2[key] + args.layer3_loss_weight * parts3[key] for key in parts1}
        ...
        if should_print:
            with torch.no_grad():
                ...
                if delta_logits2 is not None:
                    print(f"      | delta_logits2 mean={delta_logits2.mean().item():.3f} std={delta_logits2.std().item():.3f} norm={delta_logits2.norm().item():.3f}")
                    print(f"      | evidence3 mean={evidence3_logits.mean().item():.3f} std={evidence3_logits.std().item():.3f} norm={evidence3_logits.norm().item():.3f}")
                extra_stats = format_extra_attention_stats(model)
                if extra_stats is not None:
                    print(extra_stats)
                
                # Check layer 3 improvements over layer 2 and layer 1
                pred1 = output1.state_log_probs.argmax(dim=-1)
                pred2 = output2.state_log_probs.argmax(dim=-1)
                pred3 = output3.state_log_probs.argmax(dim=-1)
                ...
                exact3 = int(((pred3 == batch.target_states) | ~batch.mask).all(dim=1).sum().item())
                ...
                start_l3 = int((output3.start_posterior.argmax(dim=1) == batch.start_positions).sum().item())
                ...
                stop_l3 = int((output3.stop_posterior.argmax(dim=1) == batch.stop_transition_positions).sum().item())
                ...
                donor_l3 = int(batch.donor_mask[batch_index, output3.donor_posterior.argmax(dim=1)].sum().item())
                ...
                acceptor_l3 = int(batch.acceptor_mask[batch_index, output3.acceptor_posterior.argmax(dim=1)].sum().item())
                
                print(f"L1 vs L2 vs L3 | exact: {exact1} vs {exact2} vs {exact3} | start: {start_l1} vs {start_l2} vs {start_l3} | stop: {stop_l1} vs {stop_l2} vs {stop_l3} | donor: {donor_l1} vs {donor_l2} vs {donor_l3} | acceptor: {acceptor_l1} vs {acceptor_l2} vs {acceptor_l3}")
        if should_report:
            with torch.no_grad():
                predicted = output.state_log_probs.argmax(dim=-1)
                matches = (predicted == batch.target_states) & batch.mask
