...
GROUP_NAMES = ("U", "C", "I", "T")
GROUP_STATE_MASKS = torch.zeros(4, S, dtype=torch.bool)
for _g in range(3):
    GROUP_STATE_MASKS[0, idx_U(_g)] = True
    GROUP_STATE_MASKS[3, idx_T(_g)] = True
    for _p in range(3):
        GROUP_STATE_MASKS[1, idx_C(_g, _p)] = True
        GROUP_STATE_MASKS[2, idx_I(_g, _p)] = True
...
def one_hot_dna(sequence: str) -> torch.Tensor:
    indices = torch.tensor([DNA_TO_INDEX[b] for b in sequence], dtype=torch.long)
...
def transfer_batch(batch: DenseBatch, device: torch.device) -> DenseBatch:
    """Transfer a DenseBatch to the specified device with non-blocking copies."""
    return DenseBatch(**{
...
@torch.no_grad()
def evaluate(model: DenseTransitionPhaseModel, args: argparse.Namespace, *, device: torch.device, seed: int, examples: int, amp_enabled: bool = False) -> dict[str, object]:
    rng = random.Random(seed)
    model.eval()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prefetch", action=argparse.BooleanOptionalAction, default=True,
                        help="Prefetch next batch on a background thread")
...
def main() -> None:
    ...
    best_loss = float("inf")
    use_amp = args.amp and device.type in ("cuda", "cpu")
    ...
    print(f"torch.compile: {args.compile} (mode={args.compile_mode})")
    ...
    # Data prefetching: generate next batch on a background thread
    def _generate_batch_cpu() -> DenseBatch:
        genes = [make_gene(args, train_rng) for _ in range(args.examples_per_step)]
        return batch_to_device(genes, torch.device("cpu"))

    _prefetch_executor = ThreadPoolExecutor(max_workers=1) if args.prefetch else None
    _prefetch_future = _prefetch_executor.submit(_generate_batch_cpu) if _prefetch_executor is not None else None
    ...
    for step in range(1, args.steps + 1):
        ...
        if _prefetch_executor is not None:
            batch = transfer_batch(_prefetch_future.result(), device)
            if step < args.steps:
                _prefetch_future = _prefetch_executor.submit(_generate_batch_cpu)
        else:
            genes = [make_gene(args, train_rng) for _ in range(args.examples_per_step)]
            batch = batch_to_device(genes, device)
        ...
        validation = None
        if should_validate:
            validation = evaluate(model, args, device=device, seed=args.seed + 100000 + step, examples=args.validation_examples, amp_enabled=use_amp)
