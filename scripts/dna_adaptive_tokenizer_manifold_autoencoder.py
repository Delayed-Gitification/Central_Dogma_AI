    ...
    *,
    ...
    for _ in range(args.val_batches):
        if manifold_weight > 0 and args.manifold_family_size > 0:
            ...
            loss, rendered = loss_for_positive_pair_batch(
                model=model,
                target_a=target,
                mask_a=mask,
                lengths_a=lengths,
                ...
                latent_l2_weight=args.latent_l2_weight,
                manifold_weight=manifold_weight,
                manifold_position_weight=args.manifold_position_weight,
                manifold_temperature=args.manifold_temperature,
            )
        else:
...
def train(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"manifold_weight={args.manifold_weight}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Adaptive-token DNA SoftPack autoencoder with augmented-pair manifold loss.")
