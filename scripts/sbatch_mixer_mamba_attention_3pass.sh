#!/usr/bin/env bash
#SBATCH --job-name=mixer_mamba_attn_3p
...
python -u scripts/train_dense_transition_phase_helper_attention_extra_positional_utr_introns_local_conv_mixers_3pass.py \
  --device cuda \
  --resume-checkpoint checkpoints/curriculum_stage1_mixer_mamba_attention/best.pt \
  --checkpoint-dir checkpoints/curriculum_stage1_mixer_mamba_attention_3pass \
  --steps 8000 \
